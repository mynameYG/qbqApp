...
<script>
	...
	export default {
		...
		methods: {
			...
			getTypes(){
				this.$http.get(`/mobile/tenement/repair/types`).then(res => {
			...
			formSubmit: function(e) {
				...
				if (checkRes) {
					...
					this.$http.post('/mobile/tenement/repair/report', params).then(formData => {
						uni.showToast({title: "报修成功",icon: 'none'});
						let pages = getCurrentPages();
						let index = 0;
						for (let page of pages) {
						  if (page.route === 'PProperty/pages/service/repair-index') {
							break;
						  }
						  index++;
						} 
