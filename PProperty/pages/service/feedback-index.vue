<template>
	<view class="bg">
				...
				<view class="pl15 pr15 list-wrap">
					<view class="detail-wrap" v-for="item in list" :key="item.id" @click="navTo(item)">
						...
						<view class="detail-item flex">
							...
							<text v-if="item.status.value != 'over'" class="detail-text flex1 text-ellipsis warning">
								{{item.status.title}}
							</text>
					...
					<!-- 	<view class="detail-btn" v-if="item.status.value == 'dispatch'">
						...
						<view class="detail-btn" v-if="item.status.value == 'over' && !item.evaluateResult">
		...
		<text class="fixed-btn-rightBottom" @click="jump('/PProperty/pages/service/feedback-add')">新增</text>
	...
	export default {
		...
		methods: {
			...
			evaluate(item){
				this.info ={
					infoId:item,
					putUrl:'/mobile/tenement/feedback/evaluate'
				}
				// this.infoId = item;
			...
			getList() {
				...
				this.$http.get('/mobile/tenement/feedback/list',params).then(res => {
			...
			navTo(item) {
				uni.navigateTo({
					url:`/PProperty/pages/service/feedback-detail?id=${item.id}&status=${item.receiveStatus}`
				})
			},
			// 刷新列表
			refresh(){
