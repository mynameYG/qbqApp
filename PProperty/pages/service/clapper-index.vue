<template>
	<view class="bg">
			...
			<mix-pulldown-refresh ref="mixPulldownRefresh" :top="0" @refresh="loadData('refresh')">
				<view class="pl15 pr15 list-wrap">
					<view class="detail-wrap" v-for="item in list" :key="item.id" @click="navTo(item)">
						...
						<view class="detail-btn" v-if="item.status.value == 'closed' && item.imei == imei && !item.evaluateResult">
							<view class="btn-item"  @tap.stop="evaluate(item.id)" @refresh="refresh">评价</view>
						</view>
						<!-- <view class="detail-btn" v-if="item.status.value == 'closed'" >
							<view class="btn-item"  @tap.stop="evaluate(item.id)" @refresh="refresh">评价</view>
						</view> -->
					</view>
				</view>
		...
		<text class="fixed-btn-rightBottom" @click="jump('/PProperty/pages/service/clapper-area')">去拍</text>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$http.get('/mobile/event/list',params).then(res => {
			...
			navTo(item) {
				uni.navigateTo({
					url:`/PProperty/pages/service/clapper-detail?id=${item.id}&status=${item.receiveStatus}`
				})
			},
			// 刷新列表
			refresh(){
