				...
				<view class="pl15 pr15 list-wrap">
					<view class="detail-wrap" v-for="item in list" :key="item.id" @click="navTo(item)">
						<view class="detail-title flex flexmid">
							<view class="flex1 text-ellipsis">【{{item.type.title}}】{{item.title || '-'}}</view>
						</view>
						<template v-if="item.startDate">
							<view class="detail-item flex" v-if="item.startDate">
								<text class="detail-label">开始投票时间</text>
								<text class="detail-text flex1 text-ellipsis">{{dateFilter(item.startDate,'date') || '-'}}</text>
							</view>
							<view class="detail-item flex">
								<text class="detail-label">结束投票时间</text>
								<text class="detail-text flex1 text-ellipsis">{{dateFilter(item.endDate,'date') || '-'}}</text>
							</view>
						</template>
						<view class="detail-item flex">
							<text class="detail-label">投票状态</text>
							<text class="detail-text flex1 text-ellipsis warning">
								{{item.voteStatus.title}}
							</text>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$http.get('/mobile/tenement/vote/list',params).then(res => {
					console.log('res',res)
			...
			navTo(item) {
				uni.navigateTo({
					url:`/PProperty/pages/service/vote-detail?id=${item.id}&status=${item.receiveStatus}`
				})
			},
			// 刷新列表
			refresh(){
