	...
	export default {
		...
		onNavigationBarButtonTap(e) {
			...
			for (let page of pages) {
			  if (page.route === 'pages/contacts/address') {
				break;
			  }
			  index++;
			}
		...
		methods: {
			init(){
				this.$http.get(`/mobile/pub/community`).then(res => {
					//this.list = res;
					this.list.orgs = res;
				}).catch(err => {
			...
			ToList(item){
				uni.navigateTo({
					url: `/PProperty/pages/contacts/street-list-list?pid=${item.id}&pname=${item.name}&type=${this.type}&multi=${this.multi}`
				});
			...
			ToAdd(){
				...
				console.log('this.$storelist',`${this.type}/init`,this.persons)
