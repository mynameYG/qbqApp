	...
	<!-- 	<view class="pl15 pr15 pt10 pb10">
		...
		<view v-if="!searchShow">
			<view class="contacts-smalllist contacts-checkbox" v-if="list.users.length > 0">
				...
				<checkbox-group @change="checkboxChange" v-if="multi">
					<view class="contacts-smalllink" v-for="(item,i) in list.users" :key="i" >
						<label class="org-checkbox flex flexmid">
							<checkbox style="transform: scale(0.7);"  color="#1B6EE6" :value="item.id" :checked="users.has(item.id)" />
							<image class="contacts-smallleft" src="../../../static/img/new-address.png" mode=""></image>
				...
				<radio-group @change="checkboxChange" v-else>
					<view class="contacts-smalllink" v-for="(item,i) in list.users" :key="i" >
						<label class="org-checkbox flex flexmid">
							<radio style="transform: scale(0.7);"  color="#1B6EE6" :value="item.id" :checked="users.has(item.id)" />
							<image class="contacts-smallleft" src="../../../static/img/new-address.png" mode=""></image>
							<view class="contacts-smallmid flex1 text-ellipsis">
		...
		<view class="share-wrap">
			<view class="share-box flex flexmid">
				<view class="flex1 share-num" @click="open()">已选择：{{count}}
				<text v-show="count > 0" class="iconfont icon-xiangshang" style="color:#1B6EE6;margin: 10px;"></text>
				</view>
...
<script>	
	export default {
		...
		methods: {
			init(){
				this.$http.get(`/mobile/pub/building/${this.pid}`).then(res => {
					//this.list = res;
					this.list.users = res;
				}).catch(err => {
			...
			open(){
				if (this.count > 0) {
					uni.navigateTo({
						url: `/PProperty/pages/contacts/street-list-popup?type=${this.type}`
					});
				}
			},
			ToAdd(){
				...
				console.log('this.$store',`${this.type}/init`,this.persons)
				let pages = getCurrentPages();
				let index = 0;
				for (let page of pages) {
				  if (page.route === 'PProperty/pages/contacts/street-list') {
					break;
				  }
				  index++;
				} 
			...
			search(){
				this.$http.get('/mobile/address/search',{keyword:this.keyword}).then(res => {
					this.list.users = res;
				}).catch(err => {
					uni.showToast({title: err,icon: 'none'})
				});
...
<style lang="scss" scoped>
	@import '@/PProperty/static/css/contacts.scss';
