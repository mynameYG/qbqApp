				...
				<view class="pl15 pr15 list-wrap">
					<view class="detail-wrap" v-for="item in list" :key="item.id" @click="navTo(item)">
						<view class="detail-title flex flexmid">
							<view class="flex1 text-ellipsis">{{item.areaName || '-'}}</view>
						</view>
						<view class="detail-item flex">
							<text class="detail-label">预约状态</text>
							<text v-if="item.status.value != 'success'" class="detail-text flex1 text-ellipsis warning">
								{{item.status.title}}
							</text>
							<text v-else class="detail-text flex1 text-ellipsis success">{{item.status.title}}</text>
						</view>
						<view class="detail-btn" v-if="item.status.value != 'cancel' && item.imei == imei">
							<view class="btn-item"  @tap.stop="close(item.id)">取消预约</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$http.get('/mobile/appoint/appoints',params).then(res => {
			...
			navTo(item) {
				uni.navigateTo({
					url:`/PGov/pages/fitness/fitness-list-detail?id=${item.id}`
				})
			},
			close(item){
				var bts=["确定", "取消"];
				plus.nativeUI.confirm("确认取消预约吗？", (e)=>{  
					if(e.index == 0){
						this.$http.post(`/mobile/appoint/appoint/${item}/cancel`).then(res => {
