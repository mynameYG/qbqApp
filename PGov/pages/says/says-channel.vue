		...
		<template  v-if="channelList.length > 0" >
			<view class="my-nav p15 whiteBg no-pb">
				<view class="my-item flex flexmid arrow"  v-for="item in channelList" :key="item.id" @tap="navTo(item)">
					...
					<text class="my-text flex1">{{item.name || item.title}}</text>
				</view>
			</view>
		</template>
		<template v-else>
...
<script>
	...
	export default {
		...
		methods:{
			getChannel(){
				this.$http.get(`/mobile/direct/channelList?channelType=wyhs`).then(res => {
					console.log('ra',res)
			...
			navTo(item){
				...
				// channel.render(item)
				uni.navigateTo({
					url:`/PGov/pages/says/says-list?id=${item.id}&pageName=${item.title}&channelCode=${item.channelCode}`
				})
			}
		}
	}
