...
const util = {
	...
	// 获取地图所有类别
	getMapAllType(callBack){
		let _self = this;
			let getStoreMapType = uni.getStorageSync('mapAllType');
			if(getStoreMapType){
				callBack({
					code: getStoreMapType
				});
			}
			_self.$http.get(`/app/collection/tags`).then(res =>{
				if(getStoreMapType){
					if(JSON.stringify(getStoreMapType) == JSON.stringify(res)){
						return false
					}
				}
				callBack({
				...
				uni.setStorageSync('mapAllType', res);
			})
	},
	//拨打电话
