<template>
	<view class="map-wrap">
		...
		<!-- #endif -->
		<!-- #ifdef H5 -->
		<web-view ref="webView" :src="impweb" class="webview"></web-view>
		<!-- #endif -->
		<!-- #ifdef MP-WEIXIN -->
		<!-- 搜索 -->
		<view class="map-search">
		<text style="vertical-align: 7px;font-size:12px;">关键字:</text><input type="text" v-model="InputText" id="txtTitle"><button id="searchBtn" @tap="searchTMap(InputText,'')">搜索</button><button @tap="closeWindow()">关闭</button>
		</view> 
		...
		<view class="typeWrap">
			<view class="typeScroll" :class="typeBtnclose?'up':''">
				<view class="item" :class="typeIndex == index?'current':''" v-for="(item,index) in typeList" :key="index" @tap="searchTMap(item.title,item.code,index)">{{item.title}}</view>
			</view>
			...
			<view id="r-result" v-if="resultList.length > 0"  style="max-height:200px;overflow-y: scroll;">
				<view id='result-list'>
					<view class="list-item" :class="lastMarker.id == item.id?'current':''" v-for="item in resultList" :key="item.id" @click="markerShow(item)">
						<view class="inner">
							<text style="display:none;">{{item.lng}},{{item.lat}}</text>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.impweb = `/static/mapChannel.html?url=${url}&session=${sessionData}&pageName=在线地图`;
			// #ifdef APP-PLUS 
			...
			currentLocation(){
				...
				uni.getLocation({
					...
					success: function (res) {
						...
						self.imp=`/static/mapChannel.html?url=${self.url}&mapCenter=${[center]}&pageName=在线地图`
					}
				});
			},
		...
		getTypeList(){
			this.$http.get(`/app/collection/type`).then(res =>{
				...
				//this.typeList = res
				res.forEach(item => {
					if(this.groupType && this.groupType !='null'){
						if(item.sort == this.groupType){
							this.typeNum++;
							// searchCode.push(item.code);
							// searchText = item.title;
							// str += `<p data-code='${item.code}'>${item.title}</p>`
		...
		// 事件触发，调用接口
		searchTMap(searchText,searchCode,index){
			this.lastMarker = {};
			this.viewshow = false;
			if(!searchText){
				uni.showToast({
