			...
			<view class="model-wrap p15">
				<view class="model-box no-mb clearfix">
					<view class="model-item flex flexmid">
						<text class="model-label require">问题标题</text>
					...
					<view class="model-item">
						<view class="model-label require">问题内容</view>
						<view class="model-editText no-ml heigthAuto">
							<textarea maxlength="-1" name="content" v-model="info.content" placeholder="请输入问题内容" placeholder-class="gray-place" class="flex1 model-textarea"></textarea>
						</view>
					</view>
					<view class="model-item flex flexmid">
						<text class="model-label require">企业名称</text>
						<input class="model-editText no-ml flex1 tr" type="text"
						name='enterpriseName' v-model="info.enterpriseName"
						placeholder="请输入"
						/>
					</view>
					<view class="model-item flex flexmid">
						...
						<input class="model-editText no-ml flex1 tr" type="text"
						name='enterpriseLinkman' v-model="info.enterpriseLinkman"
						placeholder="请输入"
						/>
					</view>
					<view class="model-item flex flexmid">
						...
						name='enterprisePhone' v-model="info.enterprisePhone"
						placeholder="请输入"
						/>
					</view>
					<view class="model-item flex flexmid">
						<text class="model-label require">企业位置</text>
						<input class="model-editText no-ml flex1 tr" type="text"
	...
	export default {
		data() {
			return {
				...
				info:{
					enterpriseLinkman:this.$store.state.user.nickname,
					enterprisePhone:this.$store.state.user.mobile
				},
				address:"",
				fileList: [],//合同附件
				submitting:false,
		...
		methods: {
			...
			getTypes(){
				this.$http.get(`/mobile/business/complaint/types`).then(res => {
					//console.log('res',res)
			...
			formSubmit: function(e) {
				...
				/* 	if(params.file.length < 1){
					...
					this.$http.post('/mobile/business/complaint/signUp', params).then(formData => {
						uni.showToast({title: "提交成功",icon: 'none'});
						let pages = getCurrentPages();
						console.log('pages',pages)
						let index = 0;
						for (let page of pages) {
						  if (page.route === 'PBusiness/pages/service/business/problem-type') {
							break;
						  }
						  index++;
						} 
