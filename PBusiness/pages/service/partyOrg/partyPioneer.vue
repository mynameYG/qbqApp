		...
		<scroll-view v-if="list.length > 0"  class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadData('add')">
			<mix-pulldown-refresh ref="mixPulldownRefresh" :top="0" @refresh="loadData('refresh')">
				<view class="p15 list-wrap">
					<view class="detail-wrap" v-for="(item,index) in list" :key="item.partyMember.id"  @tap="navTo(item)">
						<view class="detail-title text-ellipsis no-mb">{{item.partyMember.name}}</view>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$http.get('/mobile/party/org/orgMemberList', params).then(res => {
			...
			navTo(item) {
				uni.navigateTo({
					url: `/PBusiness/pages/service/partyOrg/partyPioneer-detail?id=${item.partyMember.id}&name=${item.partyMember.name}`
				});
			},
			// 刷新列表
			refresh(){
