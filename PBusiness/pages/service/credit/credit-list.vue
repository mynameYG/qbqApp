		...
		<scroll-view  v-if="list.length > 0"  class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadData('add')">
			<mix-pulldown-refresh ref="mixPulldownRefresh" :top="0" @refresh="loadData('refresh')">
				<view class="model-list p15 no-mt">
					<template v-if="listCode == 'cxsfq'">
						<view class="list-item flex" @click="jumpWebPage('creditHbrb')">
							<view class="flex1">
								<view class="title text-ellipsis">示范区开展《河南省社会信用条例》集中宣传活动</view>
								<view class="info flex1 text-ellipsis color999">2021-11-09</view>
							</view>
						</view>
						<view class="list-item flex" @click="jumpWebPage('creditXhcj')">
							<view class="flex1">
								<view class="title text-ellipsis">示范区诚信企业“交房即发证”被评选为河南省“城市信用建设十大典型案例”</view>
								<view class="info flex1 text-ellipsis color999">2021-11-09</view>
							</view>
						</view>
					</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			navTo(item) {
				if(item.info.externalLink){
					this.jumpWebPage(`outSideUrl&url=${item.info.externalLink}&title=${item.info.title}`);
					return false;
				}
				uni.navigateTo({
					url: `/PBusiness/pages/service/voluntary/model-detail?id=${item.info.id}&channelCode=${this.listCode}&name=${item.info.title}`
				});
			},
			// 刷新列表
			refresh(){
